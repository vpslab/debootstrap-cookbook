...
action :create do
  ...
  unless new_resource.cache
    debootstrap(new_resource.destination_path, suite, arch, extra_packages, mirror, owner, group)
    ...
    debootstrap(tmp_dir, suite, arch, extra_packages, mirror, owner, group) unless ::File.exist? tmp_dir
...
def debootstrap(bootstrap_dir, suite, arch, extra_packages, mirror, owner, group)
  base_cmd = case suite
             when *ubuntu_suites
